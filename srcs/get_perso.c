...
/*   Updated: 2019/03/11 19:11:59 by apelissi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_perso(t_env *e, t_perso *p)
{
	...
	x = (float)p->pos_x + sin((float)p->angle / 180 * PI) * p->vd * (float)p->mv_y;
	y = (float)p->pos_y + cos((float)p->angle / 180 * PI) * p->vd * (float)p->mv_y;
	x += cos((float)p->angle / 180 * PI) * (float)p->vd * p->mv_x;
	y -= sin((float)p->angle / 180 * PI) * (float)p->vd * p->mv_x;
	p->pos_z = (p->pos_z + p->mv_z <= 0) ? p->pos_z + p->mv_z : 0;
